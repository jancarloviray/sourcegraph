...
func (g *globalDefs) Update(ctx context.Context, op store.GlobalDefUpdateOp) error {
	...
	for _, repoUnit := range repoUnits {
		...
		for _, d := range defs {
			...
			// We intentionally ignore the error here, because it is regularly
			// "violates unique constraint" because the language is already in
			// the table.
			_ = graphDBH(ctx).Insert(&dbGlobalDefLanguages{Language: l})
		}
